function [fixTex,fixRect,winRect] = MakeFixSpot_GazeAcu2(S,P,A,colorIn,colorOut,xx,yy)
...
%********** insert dots along line to xx and yy
mid = texRad;   %draw 100 dots
K = 500;
for i = 1:K
    val = (i/K) * 0.95;
    %****** compute probability dot should show, based on Gauss params
    if (val < 0.5) % closer to fixation
        dist = val * irado;   % distance from fixation
        prob = P.amp1 * exp( -(dist^2)/(2*P.sigma1*P.sigma1));
        ...
        dist = (1-val) * irado;  % distance from target
        prob = P.amp2 * exp( -(dist^2)/(2*P.sigma2*P.sigma2));
    end
    if (prob >= rand)
       xi = texRad + 1 + floor( val * S.pixPerDeg * xx  + (5*(rand-0.5)) );
       yi = texRad + 1 + floor( -val * S.pixPerDeg * yy  + (5*(rand-0.5)) );
       fixIm(yi, xi, : ) = 255;
    end
end
