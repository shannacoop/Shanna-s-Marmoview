...
ny = yy/rado;
%**********
leno = P.sigma1 * S.pixPerDeg;  % line length
wido = P.width1 * S.pixPerDeg;  % width, 0.1 vis degrees
%************
for i = 1:Kx
  for j = 1:Ky
      dx = i-cx;
      ...
      if (dist < leno)
         ...
         if (dotprod > 0.50)
           ...
           d2 = sin(theta) * dist;
%            if ( (d1 < leno) && (d2 < wido) )
%               fixIm(j,i, :) = P.cuecolor; 
%            end
           if ( (d1 > mu1) & (d1 < leno) && (d2 < wido) )
              fixIm(j,i, :) = floor( (P.cuecolor-P.bkgd)*((1-(d1/leno))^2) + P.bkgd); 
           end
         end
      end
  end
