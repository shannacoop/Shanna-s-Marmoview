...
% %e1 = e1*(P.fixPointColorOut-P.bkgd) + P.bkgd;
% 
% %************* Jude trying to make a cross here
% imo = 0.5*ones(2*texRad,2*texRad);
% xa = texRad - floor((0.300*texRad));
% xb = texRad + floor((0.300*texRad));
% ya = texRad - floor((0.800*texRad));
% yb = texRad + floor((0.800*texRad));
% imo(xa:xb,ya:yb) = 0;
% imo(ya:yb,xa:xb) = 0;   %black cross
% 
% %************
% %xa = mtexRad - floor((0.050*texRad));
% %xb = mtexRad + floor((0.050*texRad));
% %ya = mtexRad - floor((0.300*texRad));
% %yb = mtexRad + floor((0.300*texRad));
% %imo(xa:xb,ya:yb) = 1;
% %imo(ya:yb,xa:xb) = 1;   %white cross
% 
% %****************************************
% fixIm = zeros(size(imo)) + P.bkgd; %fixPointColorOut;
% fixIm(imo == 1) = P.fixPointColorOut;
% fixIm(imo == 0) = P.fixPointColorIn;
% % Ensure the image is in display ready format
% fixIm = uint8(fixIm);
% 
% % Create the fix spot texture
