...
% add elle's face to the face library... index = 1
jnk = load(fullfile('.','SupportData','GazeStimuli','ellbase.mat'));
tmp = [jnk.ellbase{1,1}; tmp];
...
% A.hCue = stimuli.circles(A.window); % circle
A.hCue = stimuli.textures(A.window); % gabor patch

A.hFbk = stimuli.circles(A.window); % big bold circle
