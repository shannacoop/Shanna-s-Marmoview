...
bandwidth=arrayfun(@(x) x.bandwdth, D.P);
bandwidths=unique(bandwidth);
nBandwidths=numel(bandwidths);
if ~isfield(P, 'numBandwidths') || P.numBandwidths==1
    ...
    n=nan(nBandwidths,1);
    for k=1:nBandwidths
        ix=bandwidth==bandwidths(k) & ~isnan(err(:));
        n(k)=sum(ix);
        tmp=err(ix);
        if isempty(tmp)
            continue
    ...
    bar(1./v, 'FaceColor', .75*[1 1 1], 'EdgeColor', 'none'); hold on
    set(A.DataPlot2, 'Xtick', 1:nBandwidths, 'XtickLabel', bandwidths)
    y=min(ylim)+.1*diff(ylim);
    for k=1:nBandwidths
        text(k-.25, y, num2str(n(k)))
    end
    hold off
    xlabel('Bandwidth')
    ylabel('Sensitivity')
